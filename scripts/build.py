...
LDFLAGS_PREFIX = "github.com/cicbyte/gig/cmd/version"
...
def build_target(goos, goarch, version, commit, build_time):
    ...
    output_name = f"gig_{goos}_{goarch}{ext}"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="gig 本地构建脚本")
    ...
    print(f"gig {version} | commit: {commit[:8]} | {build_time}")
    print()
